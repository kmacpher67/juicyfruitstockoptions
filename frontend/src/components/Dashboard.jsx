...
import { deriveRunButtonState, fetchLatestJob } from './runObsUtils';
...
const Dashboard = () => {
    ...
    // On mount: check latest job status once so the button recovers from a stale state.
    // If a job is already running server-side, start the 15-second watchdog poll.
    useEffect(() => {
        const token = localStorage.getItem('token');

        const checkLatestJob = async () => {
            const job = await fetchLatestJob(token);
            const { isRunning: serverRunning, statusLabel } = deriveRunButtonState(job);
            setRunning(serverRunning);
            setRunStatusLabel(statusLabel);

            if (serverRunning) {
                // Start watchdog if not already running
                if (!watchdogIntervalRef.current) {
                    watchdogIntervalRef.current = window.setInterval(async () => {
                        const latestJob = await fetchLatestJob(localStorage.getItem('token'));
                        const { isRunning: stillRunning, statusLabel: label } = deriveRunButtonState(latestJob);
                        if (!stillRunning) {
                            setRunning(false);
    ...
    const runAnalysis = async () => {
        setRunning(true);
        setRunStatusLabel('Analysis running...');

        // Clear any previous watchdog before starting a fresh one
        if (watchdogIntervalRef.current) {
            window.clearInterval(watchdogIntervalRef.current);
            watchdogIntervalRef.current = null;
        }
        ...
        try {
            ...
            // Primary poll: fast 2-second interval keyed to the specific job id
            const pollInterval = setInterval(async () => {
                try {
                    ...
                    if (job.status === 'completed') {
                        ...
                        clearInterval(pollInterval);
                        setRunning(false);
                        setRunStatusLabel('Previous run failed — ready to retry');
            ...
            // Watchdog: 15-second fallback that consults the latest-job endpoint.
            // Catches cases where the primary poll loses its interval (e.g. page
            // navigation bounce, timer throttling).
            watchdogIntervalRef.current = window.setInterval(async () => {
                const latestJob = await fetchLatestJob(localStorage.getItem('token'));
                const { isRunning: stillRunning, statusLabel } = deriveRunButtonState(latestJob);
                if (!stillRunning) {
                    clearInterval(pollInterval);
                    setRunning(false);
                    setRunStatusLabel(statusLabel);
                    ...
                    <div className="mb-6 flex flex-wrap items-center gap-4 bg-gray-800 p-4 rounded-lg shadow">
                        ...
                        <div className="flex flex-col items-end gap-1">
                            ...
                            {runStatusLabel && !running && (
                                <span className="text-xs text-amber-400">{runStatusLabel}</span>
                            )}
                        </div>
                    </div>
