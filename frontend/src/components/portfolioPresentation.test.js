...
test('getDetailTicker resolves underlying stock ticker for detail modal routing', () => {
    assert.equal(getDetailTicker({ underlying_symbol: 'msft' }), 'MSFT');
    assert.equal(getDetailTicker({ symbol: 'AMD 2026-04-02 202.5 Call' }), 'AMD');
    assert.equal(getDetailTicker({ symbol: 'AAPL260417C00210000' }), 'AAPL');
    assert.equal(getDetailTicker({}, ' nvda '), 'NVDA');
});
