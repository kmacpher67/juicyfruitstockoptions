...
- [ ] **Portfolio TWS Live Grid Regression Fixes**: As of 2026-03-31 the `?view=PORTFOLIO` grid shows multiple realtime rendering regressions after the TWS integration work. See `docs/features/portfolio_tws_live_grid_regressions_20260331.md`.
    ...
    - [x] **portfolio-live-grid-005**: Reviewed 2026-03-31. When realtime portfolio items overlap with the same position already present from Flex/EOD data, `?view=PORTFOLIO` now shows one merged row instead of two visible rows. The backend merge key and precedence rules prevent duplicate quantity, market value, and PnL.
