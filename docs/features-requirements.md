...
- [/] **stock-analysis-yfinance-http-rate-limit-policy**: Enforce an explicit HTTP request-rate policy for Yahoo Finance paths and keep stock-analysis in batch mode to prevent burst traffic (`429 Too Many Requests`).
    ...
    - [ ] Add scheduler-sharded execution mode (small delayed jobs) so full ticker refresh can be spread over time windows instead of one burst run.
    - [ ] Add ops settings in `system_config` for `download_batch_size`, `batch_pause_sec`, and request throttle interval to tune without code changes.
...
- [x] **Stock Analysis**: Run Live Analsis runs the live analysis of a ticker list updates the list.
    ...
    - [/] **Ticker Modal Resilience (Analysis Popup)**: Modal should remain responsive even when one or more tab endpoints are slow/unavailable.
        ...
        - [ ] **ticker-modal-resilience-004**: Add visible UI reason badges/messages for per-tab degraded states (`timeout`, `offline`, `endpoint unavailable`, `stale local data`) instead of silent empty sections.
        - [ ] **ticker-modal-resilience-005**: Add frontend regression tests for guaranteed spinner exit and settled partial-render behavior when one endpoint is intentionally delayed/failing.
    - [/] **Stock Analysis Run Observability & Stuck-Job Recovery**: Improve visibility and fail-safe behavior when **Run Live Analysis** appears stuck (button grayed out) so operators can distinguish active progress vs stalled job.
        ...
        - [ ] **stock-analysis-run-obs-004**: Wire frontend to recover grayed-out Run button from stale status using latest-job endpoint fallback and show explicit reason text (`running`, `timed_out`, `stale_watchdog_failed`, `failed`).
...
- [x] **Trade History Management**: Get entire history of trades (ie: with cost basis) and all relevant metrics
    ...
    - [/] **trade history / dividends**: Can we add dividends to the trade history view? Dividends currently come in via the NAV reports, but a dedicated report is needed for granular ticker-level detail. *(Updated 2026-04-02: backend now emits explicit dividend trade rows (`source: dividend`, `asset_class: DIV`) and TradeHistory renders them as distinct cash events.)*
