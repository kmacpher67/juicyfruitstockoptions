...
- [x] **stock-analysis-calculated-fields-contract-20260408**: Fix broken/missing calculated fields in both Analysis grid UI and downloaded XLSX contract. Ensure averages are ordered short→long in UI, add RSI + option premium/yield columns to grid, keep spreadsheet column order canonical, and harden report JSON so `NaN/Inf` values do not break rendering. *(Completed 2026-04-08: updated `StockGrid.jsx`/`Dashboard.jsx`, canonicalized `save_to_excel()` column order in `stock_live_comparison.py`, normalized dividend-yield percent handling, and hardened `/api/reports/{filename}/data` null-coercion path. Added backend/frontend/Playwright regression coverage.)*
- [x] **ui-contrast-visibility-standard-20260408**: Replace faint/washed low-opacity ticker quick-link styles (`D/G/Y`) with explicit high-contrast link colors and standardize gain/loss and ticker-health semantic colors across data grids (Analysis, Portfolio, Trades, Orders, Juicys). Threshold rule: `>50` uses success green, `<50` uses high-contrast red. Avoid low-opacity link text for actionable controls. *(Completed 2026-04-08: switched links to solid `#1976d2` family and gain/loss/health colors to `#2e7d32` / `#d32f2f` across core grid surfaces.)*
...
- [x] **portfolio-coverage-stale-tws-phantom-20260413**: Portfolio coverage should not count stale TWS-only option rows when newer Flex snapshot exists (rolled contracts must not appear as phantom naked exposure). Preserve stale TWS records in DB for audit/history, but exclude stale TWS-only rows from merged holdings used by coverage/status UI. *(Completed 2026-04-13: `_load_portfolio_holdings_rows` now detects stale TWS snapshot vs freshness threshold and limits stale TWS rows to key-matched enrichment only; stale TWS-only rows are excluded from merged output. Added regression in `tests/test_portfolio_holdings_normalization.py` for AMD-style rolled-call phantom scenario.)*
...
- [x] **Local Docker CI-Parity Runner**:
    - [x] **playwright-local-docker-parity-20260408**: Add a reproducible local Docker path for Playwright Integration Specs that bypasses host Node conflicts and matches CI runtime expectations. *(Completed 2026-04-08: added `docker-compose.e2e.yml` + `scripts/run-playwright-docker.sh`, wired `PLAYWRIGHT_IN_DOCKER=1 ./test-all.sh`, and updated `frontend/playwright.config.js` to support `PLAYWRIGHT_BASE_URL` + `PLAYWRIGHT_SKIP_WEBSERVER` for containerized execution.)*
...
- [x] **IBKR Real-Time Data — Frontend Freshness Indicator**: Show when portfolio data was last refreshed and whether live TWS is connected.
    ...
    - [/] **ibkr-tws-expiration-action-diagnostics-20260411**: Add a same-runtime diagnostics path to prove whether TWS execution ingest is actually receiving and persisting raw expiration outcome actions such as `EXPIRED` and `ASSIGNED`.
        - [x] Add service-level execution diagnostics summary (`action_counts`, recent outcome rows) so callback payloads can be inspected without querying Mongo manually.
        - [x] Extend `app/scripts/ibkr_tws_cli.py` with an `execution-diagnostics` command that refreshes executions and prints action counts from the same runtime as FastAPI.
        - [ ] Capture a real Friday/Saturday sample from live TWS and confirm whether the Trades-window `ACTION` values arrive through current execution callbacks or a different broker/API path.
        - [ ] If the live diagnostics still show no `EXPIRED` / `ASSIGNED`, investigate alternate IBKR callback/report paths and memorialize the source-of-truth mapping in docs/learning.
    - [/] **IBKR Pending Orders — Real-Time + Flex-Aware**: Add open / working order ingestion so the portfolio can show pending intent from TWS in real time and optionally blend Flex order-related history only when available. Reference: `docs/features/portfolio_pending_order_coverage.md`, `docs/learning/ibkr-available-data-and-order-models.md`.
        ...
        - [x] **ibkr-orders-014**: Orders table contract clarity for pending options/combo rows: expose explicit `Type` (`STK`/`OPT`/`BAG`) and `Sub Type` (`CALL`/`PUT`) columns, and when `secType=BAG` include IBKR combo descriptor text in the ticker label so parent rows are not reduced to underlying-only symbols. Leg rows should show any available contract symbol (not only conid). *(Completed 2026-04-08: backend adds `order_sub_type` and BAG display-symbol descriptor fallback from `combo_legs_descrip`; OrdersGrid adds `Sub Type` column and richer BAG leg labels. Regression coverage updated in `tests/test_api_orders.py` and `frontend/src/components/OrdersView.bag.test.js`.)*
...
- [x] **Stock Analysis**: Run Live Analsis runs the live analysis of a ticker list updates the list.
    ...
    - [x] **Stock Analysis-Modal Resizing & Layout Adjustments**: Make the width of the TickerDetail modal wider (up to max-w-6xl); modify Ex-Div date to only show date, and consolidate the Dividends & Fundamentals into the Core Pricing section. Answered question: "Error" field comes from any errors encountered pulling latest YFinance info during Live Comparison. *(Completed 2026-04-08)*
...
### Stock Analysis — Thorp Audit Tab
...
- [ ] **"Juicy" Opportunity Finder**:
    ...
    - [ ] **juicy-fr-followup-review-001**: Implement Follow-up/Review (`F-R`) create/update workflow for reviewed trade positions in Juicys. Define required fields (`ticker`, `trade_date`, `strategy_type`, legs, `net_credit`) and enforce `status = \"F-R\"` queue tagging. See [Juicy Follow-up/Review (F-R) Workflow](features/juicy_fr_followup_review.md), [Implementation Plan](plans/implementation_plan-20260419-juicy-fr-followup-review.md), and [Put Ratio Spreads](learning/put_ratio_spreads.md).
        - [ ] **juicy-fr-followup-review-002**: Strategy-leg contract for `BULL_PUT_RATIO_SPREAD_1X2` and `DEEP_ITM_CALL_SUBSTITUTION_PMCC`, including ratio/role validation (`1x2` puts; long high-delta call + short OTM call). Reference: [Put Ratio Spreads](learning/put_ratio_spreads.md).
        - [ ] **juicy-fr-followup-review-003**: Persist and display `effective_entry_price = entry_strike - net_premium` in the Juicys queue and item detail views. Reference: [Put Ratio Spreads](learning/put_ratio_spreads.md).
        - [ ] **juicy-fr-followup-review-004**: Add Juicys queue filter `status == \"F-R\"` with strategy sub-tabs (`Ratio Spreads`, `ITM Substitutions`). Reference: [Put Ratio Spreads](learning/put_ratio_spreads.md).
        - [ ] **juicy-fr-followup-review-005**: Support F-R MTM sync fields (`underlying_last_price`, `last_mtm_sync_at`) and roll-analysis notes; update `review_state` transitions (`Active`, `Reviewed`, `Assigned`). Reference: [Put Ratio Spreads](learning/put_ratio_spreads.md).
        - [ ] **juicy-fr-followup-review-006**: Add regression coverage for F-R create/update/filter flows in backend API and frontend Juicys views. Reference: [Put Ratio Spreads](learning/put_ratio_spreads.md).
...
| 2026-04-19 | **ADDED** | Added Juicys Follow-up/Review roadmap slice (`juicy-fr-followup-review-001..006`) with linked feature contract and phased implementation plan for F-R create/update lifecycle, queue filters, MTM sync, and regression coverage. |
