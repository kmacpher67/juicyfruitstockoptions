...
- [ ] **"Juicy" Opportunity Finder**:
    ...
    - [x] **Smart Roll / Diagonal Assistant**: Analyze existing short calls expiring within X days to find optimal rolling strategies (Calendar/Diagonal Spreads).
        ...
        - [/] **Dividend Feed**: *bug* the Holdings, Predicted Price, Target fields are empty. Is there a test for this?  The Returns field is meaningless cause it's basically the dividend amount. *(Updated 2026-04-03: UI now applies robust fallback rendering for Holdings/Predicted/Target/Return and no longer displays `$undefined`; utility regression coverage added in `frontend/src/components/dividendPresentation.test.js`. Return-definition semantics still open.)*
