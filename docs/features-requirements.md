...
- [ ] **Settings Management**:
    ...
    - [/] **setup-config-ui-stock-analysis-http-001**: Dashboard Settings should expose stock-analysis HTTP pacing and scheduler-sharding controls for admin users and include a clear enabled/disabled status indicator for sharding.
    - [ ] **User Preferences**:
...
- [ ] **Data Freshness & DB-First Read Architecture**: Make database-first reads the default contract for all market-data APIs so frontend speed and integrity do not depend on synchronous external fetches.
    ...
    - [/] **data-freshness-policy-001**: Define field-level freshness tiers and TTL/SLA windows. *(Partially complete 2026-04-07: implemented tiered thresholds in API freshness evaluator + `system_config` override contract; expanded learning doc with tier/SLA table.)*
    - [/] **data-freshness-policy-002**: Tier A (price-derived fields such as `Current Price`, `% Change`, price-based `P/E`) refreshes on short intraday cadence during market session. *(Partially complete 2026-04-07: `price_open_min`/`price_closed_min` are live and configurable.)*
    - [/] **data-freshness-policy-003**: Tier B (fundamental periodic fields such as quarterly earnings, analyst targets) refreshes daily or per-report cadence. *(Partially complete 2026-04-07: `mixed_open_min`/`mixed_closed_min` thresholds are active and configurable.)*
    - [/] **data-freshness-policy-004**: Tier C (profile/static fields such as long name, sector, industry) refreshes weekly or on-demand. *(Partially complete 2026-04-07: `profile_open_min`/`profile_closed_min` thresholds are active and configurable.)*
    - [/] **data-freshness-policy-005**: Replace fixed clock assumptions with exchange-session aware logic (market open/closed, early close, holiday calendar) when evaluating staleness. *(Partially complete 2026-04-07: freshness session logic now handles NYSE holiday closures + common early-close windows in addition to open/closed.)*
    - [/] **data-freshness-source-rule-001**: Formalize source precedence for stock/instrument freshness: prefer TWS for intraday live price/state when connected; use Flex and yfinance as fallback/backfill based on field type. *(Partially complete 2026-04-07: portfolio stats now fall back to `flex_eod` when TWS snapshot is stale; DB-first endpoints keep yfinance fallback for missing records.)*
    - [/] **data-freshness-source-rule-002**: Keep Flex authoritative for historical/EOD and audit-style records; intraday sources must not rewrite historical truth retroactively. *(Partially complete 2026-04-07: stale intraday TWS snapshots no longer override Flex EOD as canonical portfolio source.)*
    ...
    - [/] **data-ingest-scheduler-003**: Add operator settings for freshness intervals and ingest batch controls in validated config (`system_config`) instead of hardcoded constants. *(Partially complete 2026-04-07: stock-analysis scheduler sharding controls added to validated settings surface (`/api/settings/stock-analysis-http`): `scheduler_sharding_enabled`, `scheduler_shard_size`, `scheduler_shard_pause_sec`.)*
    ...
    - [/] **data-freshness-tests-003**: Add integration tests for source precedence and fallback order (TWS connected vs disconnected, Flex/yfinance fallback paths). *(Partially complete 2026-04-07: added source precedence regression for stale TWS -> Flex fallback and exchange-session holiday/early-close coverage; existing DB-missing -> yfinance fallback tests retained.)*
...
- [x] **IBKR Real-Time Data — Frontend Freshness Indicator**: Show when portfolio data was last refreshed and whether live TWS is connected.
    ...
    - [/] **IBKR Pending Orders — Real-Time + Flex-Aware**: Add open / working order ingestion so the portfolio can show pending intent from TWS in real time and optionally blend Flex order-related history only when available. Reference: `docs/features/portfolio_pending_order_coverage.md`, `docs/learning/ibkr-available-data-and-order-models.md`.
        ...
        - [/] **ibkr-orders-009**: Add IBKR Integration setting `Orders Query ID` (`query_id_orders`) and stub Flex order-history sync path; finish with concrete parser mapping once the Flex Orders report is created and verified.
...
- [x] **Stock Analysis**: Run Live Analsis runs the live analysis of a ticker list updates the list.
    ...
    - [ ] **Stock Analysis Detail loading telemetry**: Add explicit modal-level error badge/logging when detail fetch returns `found=false` so unresolved symbols can be diagnosed quickly in UI support workflows.
