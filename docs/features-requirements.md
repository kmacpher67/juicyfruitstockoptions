...
- [x] **IBKR Real-Time Data — Frontend Freshness Indicator**: Show when portfolio data was last refreshed and whether live TWS is connected.
    ...
    - [ ] **IBKR Pending Orders — Real-Time + Flex-Aware**: Add open / working order ingestion so the portfolio can show pending intent from TWS in real time and optionally blend Flex order-related history only when available. Reference: `docs/features/portfolio_pending_order_coverage.md`, `docs/learning/ibkr-available-data-and-order-models.md`.
        - [ ] **ibkr-orders-001**: Extend the realtime IBKR service to request and normalize open / working orders (`openOrder`, `orderStatus`, and related callbacks) into a dedicated pending-order shape keyed by `(account, order_id, perm_id)` where available.
        - [ ] **ibkr-orders-002**: Persist TWS working-order snapshots into a dedicated collection such as `ibkr_orders` with explicit `source: "tws_open_order"` and freshness timestamps so the UI can distinguish current pending orders from filled executions.
        ...
        - [ ] **ibkr-orders-004**: Add API support so `?view=PORTFOLIO` can fetch pending-order summaries per `(account, underlying)` group without re-implementing order matching logic in the frontend.
...
- [ ] **Portfolio View**: Add a new filter system portfolio list that allows filtering by STK/OPT groups that are not fully covered and other useful items. UI should allow user to focus useful items and take action on them.  view=PORTFOLIO page.
    ...
    - [/] **OTM / Near Money filter**: ITM In The Money & OTM Out of the Money filter for portfolio options including the underlying_symbol STK, given % distance between option strike and underlying stock price, and show all contracts that are "near" the money to focus on. Portfolio grid now supports a `Near Money (<N%)` toggle that combines with the other active portfolio filters using `AND` logic. Default threshold is `8%`, adjustable from `0` to `20`. The calculation must use underlying stock price vs option strike, not option premium vs strike.
        - [/] **Underlying STK inclusion for option filters**: When option-focused filters such as `Expiring` or `Near Money` are active, include the matching underlying stock row for each visible option group, but do not show stock rows that do not belong to one of the filtered option groups.
    - [ ] **Show "STK ?" Filter**: Add a dedicated `Show "STK ?"` portfolio filter/toggle that is checked by default. When checked, stock rows should be shown anywhere the related option group is shown; when unchecked, users may hide stock rows from the portfolio grid and related filtered views.
        - [ ] **portfolio-show-stk-001**: Default state is checked on `?view=PORTFOLIO` so STK rows are visible unless the user explicitly turns them off.
        - [ ] **portfolio-show-stk-002**: The `Show "STK ?"` toggle must combine with `Expiring`, `Near Money`, coverage status, and account filters using `AND` semantics, without breaking the inclusion of matching underlying rows for option-driven views.
        - [ ] **portfolio-show-stk-003**: When `Show "STK ?"` is checked, any filtered option result set that is shown in the grid must also include its related underlying STK row in the same account/underlying group.
        - [ ] **portfolio-show-stk-004**: When `Show "STK ?"` is unchecked, stock rows should be suppressed from the visible portfolio grid even if related options remain visible, unless a future requirement explicitly overrides that behavior.
    ...
    - [ ] **Pending Order Aware Coverage / Roll State**: The portfolio must show not only the current covered/uncovered/naked state, but also whether there are pending orders that would fill uncovered shares, buy back covered calls, or roll an existing covered-call position. Reference: `docs/features/portfolio_pending_order_coverage.md`.
        - [ ] **portfolio-pending-coverage-001**: Keep `coverage_status` as the current filled-position truth based only on actual STK and OPT positions. Pending orders must not overwrite this current-state field.
        - [ ] **portfolio-pending-coverage-002**: Add a second derived field such as `pending_coverage_effect` or `coverage_state_if_filled` that summarizes what active working orders would do for the `(account, underlying)` group. Minimum states: `none`, `covering_uncovered`, `buying_to_close`, `rolling`, `increasing_naked_risk`, `unknown`.
        - [ ] **portfolio-pending-coverage-003**: For uncovered stock, detect pending short-call sell orders that would reduce or eliminate the uncovered share gap if filled. The UI should show shares currently uncovered, shares covered now, and shares proposed to be covered by working orders.
        - [ ] **portfolio-pending-coverage-004**: For currently covered positions, detect pending `BUY` orders on the short call leg that represent buy-to-close behavior, plus paired or related sell-to-open replacement orders that represent a roll when that intent can be inferred reliably.
        - [ ] **portfolio-pending-coverage-005**: If roll intent cannot be proven from the order data, the UI should show separate pending-leg facts rather than incorrectly labeling the position as a roll. Truthful partial visibility is better than a false roll classification.
        - [ ] **portfolio-pending-coverage-006**: Portfolio rows and detail drawers should expose both current coverage state and pending-order intent at the `(account, underlying)` group level for the stock row and all related option rows.
        - [ ] **portfolio-pending-coverage-007**: Add focus filters or badges for `Pending Cover`, `Pending BTC`, and `Pending Roll` so the operator can quickly find actionable positions in `?view=PORTFOLIO`.
        - [ ] **portfolio-pending-coverage-008**: Add or refresh regression coverage for representative cases: uncovered shares with one pending short call sell order, covered call with pending buy-to-close only, covered call with buy-to-close plus replacement short call roll, and ambiguous order sets that must remain `unknown`.
    ...
    - [/] **Export**: Export current view of portfolio to CSV (inclusive of filters).
        - [ ] **portfolio-export-001**: CSV export must use the same final visible-row set as the current `?view=PORTFOLIO` grid after all active filters and toggles are applied.
        - [ ] **portfolio-export-002**: When option rows are exported with `Show "STK ?"` checked, the export must also include the corresponding STK row for each exported option group so the CSV does not omit the underlying stock context.
        - [ ] **portfolio-export-003**: When `Show "STK ?"` is unchecked, the CSV export should omit STK rows in the same way the grid omits them.
