...
- [x] **Stock Analysis**: Run Live Analsis runs the live analysis of a ticker list updates the list.
    ...
    - [/] **Stock Analysis Data Persistence**: Persist complete per-ticker analysis payloads used by detail APIs so runs/schedules and modal retrieval stay aligned.
        - [ ] **stock-analysis-db-001**: On manual and scheduled Run Live Analysis, persist the full analysis field set per ticker into `stock_data` (not only spreadsheet output).
        - [ ] **stock-analysis-db-002**: Persist profile/news detail fields required by ticker modal tabs so hydration/fallback is deterministic and not dependent on live external fetches.
        - [ ] **stock-analysis-db-003**: Add persistence completeness regression tests that assert required field presence for ticker detail endpoints after ingestion.
