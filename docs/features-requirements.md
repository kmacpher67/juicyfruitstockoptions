...
- [x] **IBKR Real-Time Data — Frontend Freshness Indicator**: Show when portfolio data was last refreshed and whether live TWS is connected.
    ...
    - [/] **IBKR Pending Orders — Real-Time + Flex-Aware**: Add open / working order ingestion so the portfolio can show pending intent from TWS in real time and optionally blend Flex order-related history only when available. Reference: `docs/features/portfolio_pending_order_coverage.md`, `docs/learning/ibkr-available-data-and-order-models.md`.
        ...
        - [ ] **ibkr-orders-012**: BAG/combo leg decomposition in Orders UI. Current table can show BAG parent rows but does not yet expose per-leg drill-down from `comboLegs`/contract-leg metadata; implement grouped roll view with explicit leg rows (BUY-to-close leg, SELL-to-open leg, ratios, and net debit/credit context).
        - [ ] **ibkr-orders-013**: Add BAG-parent visibility controls (`Show BAG parents`, `Show decomposed legs only`) so users can choose between net combo rows vs individual-leg execution context.
