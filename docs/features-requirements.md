...
- [x] **Stock Analysis**: Run Live Analsis runs the live analysis of a ticker list updates the list.
    ...
    - [x] **Stock Analysis-Optimizer**: Multi-leg strategy optimizer in Optimizer tab — risk/reward visualization, what-if scenario analysis (price up/down), and yield comparison across strategies. Related: [Smart Roll & Diagonal](learning/smart-roll-diagonal.md). *(Completed 2026-04-07: optimizer now uses persisted juicy candidate rows with score/yield fields and table-first rendering.)*
        ...
        - [ ] **stock-analysis-optimizer-juicy-table-009-option-chain-depth**: Expand per-ticker juicy generation beyond the current top-level heuristic rows into option-chain level candidates. For each of the next `4` DTE dates, evaluate `1` nearest ITM call and `4` nearest OTM calls for sale; persist each candidate with `premium`, `yield_pct`, `annualized_yield_pct`, `dte`, and strike-distance metadata.
        - [ ] **stock-analysis-optimizer-juicy-table-010-liquidity-qualifier**: Add liquidity/timeframe qualifiers to optimizer scoring so chain candidates are graded by daily/weekly/monthly tradability quality. Minimum tracked fields: `volume`, `open_interest`, `bid_ask_spread`, `spread_pct_mid`, and a derived `liquidity_grade` (`A/B/C/D`) used in final score ranking and UI badges.
        - [ ] **stock-analysis-optimizer-juicy-table-011-short-dte-focus**: Add explicit short-DTE ranking mode so high-yield short duration candidates are surfaced first but penalized when liquidity quality is weak or spread cost dominates premium edge.
...
- [ ] **Top Navigation — Juicys Workspace**:
    - [ ] **juicys-nav-001**: Add a new top-level navigation tab labeled `Juicys` in the main dashboard nav (alongside Analysis/My Portfolio/Trade History/Orders).
    - [ ] **juicys-nav-002**: Route `Juicys` to a dedicated spreadsheet-style workspace showing all tracked analysis tickers and their current Juicy opportunities from Mongo persistence (not live-only ephemeral results).
    - [ ] **juicys-nav-003**: Workspace defaults to "Juicy main" view and displays all Juicy items; include filter presets for "Juicy Fruit Options" and "Hot PUTS on down market days".
    - [ ] **juicys-nav-004**: Juicys grid should support column sorting/filtering/search with ticker-first layout consistent with existing data tables.
    - [ ] **juicys-nav-005**: Add workspace `Refresh` action that schedules the Juicy refresh job and shows queue/freshness status instead of blocking on synchronous fetches.
    ...
    - [ ] **juicys-nav-007**: Create and maintain feature detail doc [Juicys Navigation + Optimizer Workspace](features/juicys_navigation_optimizer_workspace.md) as the implementation contract.
