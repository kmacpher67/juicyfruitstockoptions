...
- [ ] **"Juicy" Opportunity Finder**:
    ...
    - [ ] **juicy-fr-followup-review-001**: Implement Follow-up/Review (`F-R`) create/update workflow for reviewed trade positions in Juicys. Define required fields (`ticker`, `trade_date`, `strategy_type`, legs, `net_credit`) and enforce `status = \"F-R\"` queue tagging. See [Juicy Follow-up/Review (F-R) Workflow](features/juicy_fr_followup_review.md) and [Implementation Plan](plans/implementation_plan-20260419-juicy-fr-followup-review.md).
        - [ ] **juicy-fr-followup-review-002**: Strategy-leg contract for `BULL_PUT_RATIO_SPREAD_1X2` and `DEEP_ITM_CALL_SUBSTITUTION_PMCC`, including ratio/role validation (`1x2` puts; long high-delta call + short OTM call).
        - [ ] **juicy-fr-followup-review-003**: Persist and display `effective_entry_price = entry_strike - net_premium` in the Juicys queue and item detail views.
        - [ ] **juicy-fr-followup-review-004**: Add Juicys queue filter `status == \"F-R\"` with strategy sub-tabs (`Ratio Spreads`, `ITM Substitutions`).
        - [ ] **juicy-fr-followup-review-005**: Support F-R MTM sync fields (`underlying_last_price`, `last_mtm_sync_at`) and roll-analysis notes; update `review_state` transitions (`Active`, `Reviewed`, `Assigned`).
        - [ ] **juicy-fr-followup-review-006**: Add regression coverage for F-R create/update/filter flows in backend API and frontend Juicys views.
