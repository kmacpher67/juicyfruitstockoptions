...
- [x] **IBKR Real-Time Data — Frontend Freshness Indicator**: Show when portfolio data was last refreshed and whether live TWS is connected.
    ...
    - [ ] **ibkr-tws-portfolio-rt-dedupe-001**: Integrate realtime portfolio items into `?view=PORTFOLIO` with duplicate-safe row merging. When the same open position or intraday trade signal is present from both Flex/EOD and TWS/live sources, the portfolio grid must render one visible row, not two, and prefer the freshest live fields without double-counting quantity, value, or PnL.
